...
/* 
    ...
    /*
    String reservationNum = "select reservationNum from reservation where dateReserve = '"+ currentDate +"' AND passenger = '" +name+ "';";	
    rs = st.executeQuery(reservationNum);
    rs.next();
	String reservationNumInt = rs.getString(1);
	double rNI = Integer.parseInt(reservationNumInt);
    */			
    //int execute_two = stmt.executeUpdate("insert into reserves(reservationNum, username) values( "+rNI+" ,'" +username+"')");    
    ...
    //int execute_three = stmt.executeUpdate("insert into matches(reservationNum, trainID, tLine_name, origin, destination) values( "+rNI+", "+trainID+",'" +tLine_name+ "', '" +origin+"', '"+destination+"')");
