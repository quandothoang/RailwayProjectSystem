...
<form method="get" action="trainScheduleDBChange.jsp">
	<table>

		<tr>
			<td>Departure time change:</td><td><input type="text" name="dTimeChange"></td>
				<td>format: minutes or -minutes</td>
		</tr>

		<tr>
			<td>base fare ($/0 stops):</td><td><input type="text" name="nFare"></td>
		</tr>
...
ResultSet rs;
String query;
if(!(trainID.equals("")) && !(tLine_name.equals("")) && !(date.equals(""))){
	query = "select * from train_schedule where trainID='" +trainID+  "' AND tLine_name='"+tLine_name+"' AND dTime > (select cast('"+date+"' AS datetime)) AND dTime < (select cast((SELECT DATE_ADD('"+date+"', INTERVAL 1 DAY)) AS datetime));";
